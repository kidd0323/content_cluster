...
int main(int argc, char* argv[])
{
	...
	while (! feof(taskFileHandle))
	{
		...
		fscanf(taskFileHandle,"%s",image_file_name);
			...
			vector[NY+NC+2] = Cr[2]*1.41421;
 			...
 			for( int i= NC+NY+3;i< 2*NC+NY;i++ )
				vector[i] = Cr[i-NC-NY];
                        ...
                        //printf("%d\n",bestOrder);
                        char saveName[255];
                        sprintf(saveName,"./Image/%d/%d.jpg",bestOrder,num);
                        cvSaveImage(saveName,test);
		}
		cvReleaseImage(&test);
		delete cl;
		delete frame;
